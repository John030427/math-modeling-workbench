  ...
  return (
    <div className="mm-panel">
      <div style={{ display: 'flex', flexWrap: 'wrap', gap: 8, marginBottom: 12, alignItems: 'center' }}>
        ...
        <button className="mm-btn ghost" type="button" onClick={() => setSeed((s) => s + 1)}>
          随机初始化
        </button>
        <button className="mm-btn ghost" type="button" onClick={next} disabled={phase === 'done'}>
          下一步
        </button>
        <button className="mm-btn" type="button" onClick={() => setAuto((a) => !a)}>
          {auto ? '暂停' : '自动运行'}
        </button>
        <span className="mm-chip">step {step} · {phase}</span>
      </div>
      <canvas ref={canvasRef} width={600} height={400} style={{ width: '100%', maxWidth: '100%' }} />
