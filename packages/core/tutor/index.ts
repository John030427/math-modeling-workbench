...
export function routeSkill(
  ...
  if (onLesson && conceptual) return '01-tutor'
  if (
    ['缺失', '异常', '预处理', 'outlier', 'missing'].some((k) => msg.includes(k)) ||
    (['标准化', '量纲', '填充'].some((k) => message.includes(k)) && !onLesson)
  ) {
    return '12-data-doctor'
  }
  if (['特征', 'feature', 'lag', '比率'].some((k) => msg.includes(k))) return '13-feature-engineering'
  if (['选模型', '用什么模型', 'model select', '该用'].some((k) => msg.includes(k))) return '20-model-selector'
  if (['论文', '摘要', '写作', 'paper'].some((k) => msg.includes(k))) return '40-paper-writer'
  if (['评分', '评审', 'review', '扣分'].some((k) => msg.includes(k))) return '41-paper-reviewer'
  if (['差距', '薄弱', '不会', 'gap'].some((k) => msg.includes(k))) return '42-gap-analyzer'
  if (pg.includes('gym') || ['变量', '约束', '目标函数', '决策'].some((k) => msg.includes(k))) {
    return '11-modeling-coach'
  }
  if (onLesson) return '01-tutor'
  return '00-router'
}
...
export function buildTutorPrompt(ctx: TutorContext): string {
  ...
  if (ctx.model?.use_when?.length) parts.push(`use_when: ${ctx.model.use_when.join('; ')}`)
  if (ctx.model?.avoid_when?.length) parts.push(`avoid_when: ${ctx.model.avoid_when.join('; ')}`)
...
export function offlineReply(ctx: TutorContext): TutorReply {
  ...
  if (skill === '12-data-doctor') {
    return {
      ...
      mode,
      answer:
        '处理缺失前请按链路思考：缺失比例 → 变量类型 → 是否时间/空间结构 → MCAR/MAR/MNAR → 是否携带信息 → 再选 Mean/Median/插值/KNN/MICE/Indicator/Drop。不要默认均值填充。',
  ...
  if (skill === '20-model-selector') {
    return {
      ...
      mode,
      answer:
        '模型选择顺序：先判定任务类型与约束结构，再给 Baseline / Main / Alternative，并解释线性可解时为何不要默认 PSO。',
      offline: true,
    }
  }
  ...
  return {
    ...
    mode,
    answer:
      '我是工作台助手。可以结合当前页面帮你学算法、拆题、做数据诊断或论文评审。请告诉我你在哪个模块，或打开具体模型课程。',
    offline: true,
  }
}

function kmeansTutor(message: string, mode: string, ku: string | null | undefined): TutorReply {
  const msg = message.toLowerCase()
  if (['标准化', '归一化', 'scaling', 'scale', '量纲'].some((k) => message.includes(k))) {
    let answer =
      '因为 K-Means 用欧氏距离。若收入是 3000–100000、年龄是 18–65，收入会主导距离，年龄几乎不起作用。标准化让各维可比。DBSCAN 同样基于距离/密度，通常也需要先处理量纲。'
    if (mode === 'coach') {
      answer =
  ...
  if (msg.includes('dbscan')) {
    return {
      ...
      mode,
      answer:
        'K-Means 假设簇大致球形并需指定 K；DBSCAN 基于密度，可找不规则簇并标记噪声，但不保证每个点都有簇标签，且对 eps/min_samples 敏感。',
  ...
  if (['sse', '平方误差'].some((k) => message.toLowerCase().includes(k)) || ku === 'sse') {
    return {
      ...
      mode,
      answer:
        'SSE（簇内平方误差）= Σ‖xi − c_zi‖²：每个样本到其簇中心的欧氏距离平方之和。K-Means 迭代就是在最小化 SSE。',
  ...
  if (['为什么', '直觉', '干什么'].some((k) => message.includes(k))) {
    let base =
      'K-Means 解决的是：把相似样本分到同一组，使组内更紧、组间更分离（以到中心距离衡量）。'
    if (mode === 'coach') {
...
function genericModelTutor(model: ModelDetail, message: string, mode: string): string {
...
function coachReply(message: string, mode: string): TutorReply {
  const steps = [
    '这句话里，什么是你能控制的决策变量？',
